...
type ImageGenerationHistoryCreateInput = Omit<
...
function normalizeItem(value: unknown): ImageGenerationHistoryItem | null {
  ...
  const images = Array.isArray(record.images)
    ? record.images
        ...
        .filter((item): item is ImageGenerationHistoryItem["images"][number] =>
          Boolean(item),
        )
    : [];
...
function normalizeState(value: unknown): ImageGenerationHistoryState {
  ...
  return {
    version: 1,
    items: (items as ImageGenerationHistoryItem[]).sort(
...
export class ImageGenerationHistoryService {
  private writeQueue: Promise<ImageGenerationHistoryState> = Promise.resolve({
    version: 1,
    items: [],
  });
  ...
  async create(
    input: ImageGenerationHistoryCreateInput,
  ...
  async delete(
    idValue: unknown,
  ): Promise<{ deleted: boolean; items: ImageGenerationHistoryItem[] }> {
