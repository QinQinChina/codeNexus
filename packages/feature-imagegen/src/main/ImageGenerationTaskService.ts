  ...
  return {
    ...
    inputImages: Array.isArray(record.inputImages)
      ? record.inputImages
          ...
          .filter((item): item is { dataUrl: string; name: string | null } =>
            Boolean(item),
          )
      : null,
    ...
    outputCompression:
      typeof record.outputCompression === "number"
...
function normalizeTask(value: unknown): ImageGenerationTaskItem | null {
  ...
  const persistedStatus =
    status === "running" || status === "queued" ? "failed" : status;
  const now = Date.now();
  return {
    id,
...
function sanitizeTaskArgsForSnapshot(
  args: ImageGenerationGenerateArgs,
): ImageGenerationGenerateArgs {
  return {
    ...args,
...
function normalizeState(value: unknown): ImageGenerationTaskState {
  ...
  return {
    version: 1,
    tasks: (tasks as ImageGenerationTaskItem[]).sort(
...
export class ImageGenerationTaskService {
  ...
  async cancel(idValue: unknown): Promise<{
    canceled: boolean;
  ...
  async delete(
    idValue: unknown,
  ): Promise<{ deleted: boolean; tasks: ImageGenerationTaskItem[] }> {
    await this.ensureLoaded();
    const id = toText(idValue);
  ...
  async retry(idValue: unknown): Promise<{
    task: ImageGenerationTaskItem;
    tasks: ImageGenerationTaskItem[];
  }> {
    await this.ensureLoaded();
    ...
    if (source.status === "queued" || source.status === "running") {
      throw new Error(
        "Image generation task is in progress and cannot be retried.",
      );
    }
    const now = Date.now();
    const task: ImageGenerationTaskItem = {
    ...
    this.tasks = [
      task,
      ...this.tasks.filter((_, index) => index !== sourceIndex),
    ];
    await this.persist();
    this.pump();
    return { task: { ...task }, tasks: this.snapshot() };
  ...
  private async persist(): Promise<void> {
    const task = this.saveQueue.then(async () => {
      ...
      await writeFile(
        this.filePath,
        `${JSON.stringify({ version: 1, tasks: this.tasks }, null, 2)}\n`,
        "utf8",
      );
    });
