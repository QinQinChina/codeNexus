<template>
  <aside
    class="sidebar sidebar-left image-workspace-sidebar"
    :aria-label="t('imageWorkspace.aria')"
  >
            ...
            <div class="lsb-pane-head-row">
              <div class="image-workspace-title-block">
                ...
                <div class="image-workspace-title-copy">
                  <h2 class="lsb-pane-title">
                    {{ t("imageWorkspace.title") }}
                  </h2>
                  <div class="image-workspace-summary mono">
                    {{
                      t("imageWorkspace.recordCount", {
                        count: workbench.historyItems.length,
                      })
                    }}
                  </div>
                </div>
          ...
          <div class="lsb-scroll image-workspace-scroll app-scrollbar">
            ...
            <template v-else>
              <section
                ...
                :key="group.key"
                class="image-workspace-group"
              >
                <button
                  class="image-workspace-group__head"
                  type="button"
                ...
                >
                  ...
                  />
                  <span
                    class="image-workspace-group__title"
                    :title="group.path || group.label"
                    >{{ group.label }}</span
                  >
                  <span class="image-workspace-group__count mono">{{
                    group.items.length
                  }}</span>
                </button>
                ...
                >
                  <article
                    ...
                    :class="[
                      `is-${statusKind(item)}`,
                      {
                        'is-selected': item.id === workbench.selectedHistoryId,
                  ...
                  >
                    ...
                    <div class="image-workspace-item__body">
                      <div class="image-workspace-item__meta">
                        <span class="image-workspace-item__time mono">{{
                          formatTime(item.createdAt)
                        }}</span>
                        <span class="image-workspace-item__status">{{
                          statusLabel(item)
                        }}</span>
                      </div>
                      <div class="image-workspace-item__prompt">
                        {{ item.prompt }}
                      </div>
                    </div>
...
import {
  ...
  useImagegenWorkspacePathRef,
} from "../runtimeBridge";
import {
  useImageWorkbenchStore,
...
const groups = computed<ImageWorkspaceGroup[]>(() => {
  ...
  return [...byKey.values()]
    ...
    .sort((a, b) => {
      ...
      if (
        a.key === UNASSIGNED_WORKSPACE_KEY &&
        b.key !== UNASSIGNED_WORKSPACE_KEY
      )
        return 1;
      if (
        b.key === UNASSIGNED_WORKSPACE_KEY &&
...
function statusLabel(item: ImageWorkbenchHistoryItem): string {
  ...
  if (status === "pending")
    return item.errorText || t("imageWorkbench.generating");
...
function toggleGroup(key: string) {
  collapsedByKey.value = {
    ...collapsedByKey.value,
    [key]: !collapsedByKey.value[key],
  };
}
...
async function ensureThumb(pathValue: string) {
  ...
  if (!path || thumbByPath.value[path] || thumbLoadingByPath.value[path])
    return;
...
.image-workspace-shell {
  background:
    linear-gradient(
      180deg,
      color-mix(in srgb, var(--lsb-bg-strong) 96%, transparent),
      var(--lsb-bg)
    ),
    var(--lsb-bg);
}
...
.image-workspace-head {
  ...
  background:
    radial-gradient(
      circle at 16px 12px,
      color-mix(in srgb, var(--lsb-accent) 12%, transparent),
      transparent 30px
    ),
    linear-gradient(
      180deg,
      color-mix(in srgb, var(--lsb-bg-strong) 98%, transparent),
      var(--lsb-bg)
    );
}
...
.image-workspace-head::after {
  ...
  background: linear-gradient(
    90deg,
    transparent,
    color-mix(in srgb, var(--lsb-accent) 32%, transparent),
    transparent
  );
  pointer-events: none;
}
