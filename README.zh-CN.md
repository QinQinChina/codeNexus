...
| 包管理器       | `pnpm@10`                         |
| Codex CLI 基线 | `@openai/codex@0.131.0`           |
...
| 路径           | 用途                                    |
| -------------- | --------------------------------------- |
| `src/main`     | Electron 主进程、窗口、IPC 和服务编排。 |
| `src/preload`  | 安全的 `contextBridge` preload 层。     |
| `src/renderer` | Vue 界面、Pinia 状态和运行时协调逻辑。  |
| `src/shared`   | 跨进程契约和协议类型。                  |
| `scripts`      | 开发、构建、图标和工具脚本。            |
| `music`        | 内置通知音频资源。                      |
