<template>
  <div class="center-empty-state flex flex-col w-full max-w-[860px] mx-auto pt-[8vh] pb-10 px-6 max-[1500px]:max-w-[720px] max-[1500px]:pt-[6vh] animate-enter-pop">
    ...
    <div v-if="mode === 'default' && !loading" class="flex flex-col items-center text-center mb-12 animate-enter-slide-up">
      ...
      <p class="text-sm max-[1500px]:text-[13px] text-[var(--text-muted)] mb-8">选择一个历史记录继续，或开启全新的探讨</p>
    ...
    <template v-else>
      ...
      <div v-if="mode === 'pendingThread'" class="center-empty-pending flex flex-col items-center gap-4 my-10">
        ...
        <div class="flex items-center justify-center gap-2 bg-[var(--surface-2)] px-4 py-2 rounded-full border border-[var(--border)]">
          <LoadingDots class="mono dim text-[13px]" baseText="正在构建线程上下文" :intervalMs="350" :maxDots="3" />
        </div>
      </div>

      <!-- 空线程已创建 -->
      <div v-else-if="mode === 'emptyThread'" class="center-empty-pending flex flex-col items-center gap-5 my-10 animate-enter-fade">
        <PendingThreadArt class="center-empty-pending-art w-[240px] max-[1500px]:w-[200px]" />
        <div class="flex flex-col items-center gap-4">
          <div class="mono dim text-[13px] bg-[var(--surface-2)] px-4 py-1.5 rounded-full border border-[var(--border)]">⚡ 线程就绪</div>
          <button
            type="button"
      ...
      <!-- 历史对话网格 -->
      <div v-else-if="historyItems.length > 0" class="center-empty-history w-full animate-enter-slide-up" style="animation-delay: 100ms;">
        <div class="flex items-center justify-between mb-4 px-1">
          ...
          <span class="text-[12px] max-[1500px]:text-[11px] text-[var(--text-muted)] opacity-60">最近 {{ historyItems.length }} 条</span>
        </div>
        ...
        <div class="grid grid-cols-2 gap-3 max-[1500px]:gap-2.5">
          ...
          >
            <span class="title font-medium text-[14px] max-[1500px]:text-[13px] text-[var(--text)] group-hover:text-[var(--accent)] transition-colors w-full truncate mb-1">
              {{ item.title }}
            </span>
            <span class="text-[11px] max-[1500px]:text-[10px] text-[var(--text-muted)] font-mono opacity-70 flex items-center gap-1">
