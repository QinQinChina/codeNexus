<template>
  <section class="image-workbench" :aria-label="t('imageWorkbench.aria')">
    <div v-if="workbench.historyLoading && !selectedHistoryItem" class="image-workbench__empty">
      <Loader2 class="image-workbench__empty-icon is-spinning" aria-hidden="true" />
      <div>{{ t("imageWorkbench.loadingHistory") }}</div>
    </div>

    <div v-else-if="selectedHistoryItem && selectedImage" class="image-workbench__viewer">
      <header class="image-workbench__viewer-head">
        <div class="image-workbench__viewer-copy">
          <div class="image-workbench__viewer-kicker mono">{{ formatDateTime(selectedHistoryItem.createdAt) }}</div>
          <h2>{{ selectedHistoryItem.prompt }}</h2>
        </div>
        <div class="image-workbench__viewer-actions">
          <span class="image-workbench__zoom mono">{{ Math.round(getImageZoom(selectedImage.path) * 100) }}%</span>
          <button
      ...
      <div v-if="selectedHistoryItem.images.length > 1" class="image-workbench__filmstrip app-scrollbar">
        <button
    ...
    <div v-else class="image-workbench__empty">
      ...
      <div>
        {{
          workbench.historyItems.length > 0 ? t("imageWorkbench.selectFromWorkspace") : t("imageWorkbench.emptyHistory")
        }}
      </div>
    </div>
  </section>
...
import { Copy, Download, Image as ImageIcon, Loader2, RotateCcw, Trash2, ZoomIn, ZoomOut } from "lucide-vue-next";
...
const selectedImage = computed(
  () => selectedHistoryItem.value?.images[selectedImageIndex.value] ?? selectedHistoryItem.value?.images[0] ?? null
);
...
function setImageZoom(path: string, zoom: number) {
  imageZoomByPath.value = { ...imageZoomByPath.value, [path]: clampNumber(zoom, MIN_ZOOM, MAX_ZOOM) };
}
...
async function ensureImageDataUrl(path: string) {
  ...
  try {
    const dataUrl = await readLocalImageDataUrl(normalized);
    imageDataUrlByPath.value = { ...imageDataUrlByPath.value, [normalized]: dataUrl };
  } catch (error: any) {
...
watch(
  () => [workbench.selectedHistoryId, workbench.historyItems.map((item) => item.id).join("\n")] as const,
